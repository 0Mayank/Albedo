...
async def check_priv(ctx, member):
    try:
        ...
        if member == ctx.author:
            await ctx.send(f"You can't {ctx.command.name} yourself, although I want to")
            return False
        if member.id == ctx.bot.user.id:
            await ctx.send("So that's what you think of me huh..? You pathetic humans")
            return False

        # Protect the almins
        if member.id in almins:
            if ctx.author.id not in almins:
                await ctx.send(f"You dare {ctx.command.name} my lords (●'◡'●)")
                return False
            else:
                await ctx.send(f"I am sorry, i cannot betray either of my lords")
                return False
        ...
        if member.id == ctx.guild.owner.id:
            await ctx.send(f"Even if i tried, i will not be able to {ctx.command.name} the server owner, sed")
            return False
        if ctx.author.top_role == member.top_role:
            await ctx.send(f"You can't {ctx.command.name} someone who has the same permissions as you...")
            return False
        if ctx.author.top_role < member.top_role:
            await ctx.send(f"Nope, you can't {ctx.command.name} someone higher than yourself.")
            return False
        return True
    except Exception:
        return False
